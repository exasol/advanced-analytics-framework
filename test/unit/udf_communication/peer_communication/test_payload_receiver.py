...
def create_acknowledge_payload_message(
...
def test_received_payload_in_reverse_sequence(number_of_messages: int):
    ...
    test_setup.payload_receiver.received_payload(message, frames)
    acknowledge_message = create_acknowledge_payload_message(test_setup, message)
    send_of_previous_messages = [
        call.send_multipart(frames) for frames in reversed(frames_of_previous_message)
    ]
    out_control_mock_calls = [call.send_multipart(frames)] + send_of_previous_messages
    assert (
        test_setup.out_control_socket_mock.mock_calls == out_control_mock_calls
        and test_setup.sender_mock.mock_calls
        == [call.send(message=acknowledge_message)]
    )


@pytest.mark.parametrize(
    "number_of_messages, duplicated_message",
    [(i, j) for i in range(1, 10) for j in range(0, i)],
)
def test_received_payload_in_sequence_multiple_times(
...
@pytest.mark.parametrize("number_of_messages", [i for i in range(1, 10)])
def test_is_ready_to_stop_after_received_payload_in_reverse_sequence(
...
@pytest.mark.parametrize(
    "number_of_messages, duplicated_message_sequence_number",
    [(i, j) for i in range(1, 10) for j in range(0, i)],
)
def test_is_ready_to_stop_payload_in_sequence_multiple_times(
    number_of_messages: int, duplicated_message_sequence_number: int
):
    test_setup = create_test_setup()
    for sequence_number in range(number_of_messages):
        message, frames = create_payload_message(test_setup, sequence_number)
...
def test_is_ready_to_stop_after_received_payload_in_reverse_sequence_incomplete(
    number_of_messages: int,
):
    test_setup = create_test_setup()
    frames_of_previous_message = []
