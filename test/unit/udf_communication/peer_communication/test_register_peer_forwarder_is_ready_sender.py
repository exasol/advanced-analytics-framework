...
def create_test_setup(
    ...
    out_control_socket_mock: Socket | MagicMock = create_autospec(Socket)
    register_peer_forwarder_is_ready_sender = RegisterPeerForwarderIsReadySender(
        peer=peer,
...
@pytest.mark.parametrize(
    "needs_to_send_acknowledge_register_peer,needs_to_send_register_peer",
    [(True, True), (True, False), (False, True), (False, False)],
)
def test_init(
    needs_to_send_acknowledge_register_peer: bool, needs_to_send_register_peer: bool
):
...
def test_try_send_after_init(
    ...
    if expected_to_send:
        assert test_setup.timer_mock.mock_calls == [
            call.is_time()
        ] and test_setup.out_control_socket_mock.mock_calls == [
            call.send(
                serialize_message(
                    messages.PeerRegisterForwarderIsReady(peer=test_setup.peer)
                )
            )
        ]
    else:
