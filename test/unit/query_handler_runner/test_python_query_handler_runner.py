...
class StartFinishTestQueryHandler(QueryHandler[TestInput, TestOutput]):
    def __init__(
        self, parameter: TestInput, query_handler_context: ScopeQueryHandlerContext
    ):
...
def test_start_finish(context_mock):
    """
    This test runs a query handler which returns a Finish result from the start method.
...
def test_start_finish_cleanup_queries(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which registers a temporary table in the start method
    ...
    sql_executor = create_sql_executor(
        ...
        prefix,
        expect_query('DROP TABLE IF EXISTS "{schema}"."{prefix}_1";'),
    )
...
def test_continue_finish(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which returns Continue result from the start method
...
def test_continue_wrong_columns(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which returns Continue result with mismatching column definition
...
def test_continue_error_cleanup_queries(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which registers a temporary table in the handle_query_result method
    and then directly raise an exception. We expect a cleanup query for the temporary
    table to be executed and the exception to be forwarded.
    """
...
def test_continue_continue_finish(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which returns Continue from the first call to handle_query_result method
...
class ContinueContinueCleanupFinishTestQueryHandler(
...
def test_continue_cleanup_continue_finish(aaf_pytest_db_schema, prefix, context_mock):
    """
    This test runs a query handler which creates the temporary table of a child query context manager.
    ...
    sql_executor = create_sql_executor(
        ...
        expect_query('DROP VIEW IF EXISTS "{schema}"."{prefix}_6_1";'),
    )
    test_input = TestInput()
    query_handler_runner = PythonQueryHandlerRunner[TestInput, TestOutput](
        sql_executor=sql_executor,
...
def test_fail_in_cleanup(aaf_pytest_db_schema, context_mock):
    ...
    with pytest.raises(
        RuntimeError, match="Execution of query handler .* failed."
    ) as e:
