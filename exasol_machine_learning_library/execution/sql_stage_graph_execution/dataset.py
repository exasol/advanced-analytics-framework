...
class Dataset:
    ...
    def _check_table_name(self):
        all_table_like_names = {data_partition.table_like.name
                                for data_partition in self.data_partitions.values()}
        if len(all_table_like_names) != len(self.data_partitions):
            raise ValueError("The names of table likes of the data partitions should be different.")

    def _check_columns(self):
        all_columns = {column for data_partition in self.data_partitions.values()
