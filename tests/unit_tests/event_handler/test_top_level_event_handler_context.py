...
def test_cleanup_invalid_temporary_table_proxies_after_release(
    ...
    queries = top_level_event_handler_context.cleanup_released_object_proxies()
...
def test_cleanup_invalid_temporary_view_proxies_after_release(
        top_level_event_handler_context: TopLevelEventHandlerContext):
    proxy = top_level_event_handler_context.get_temporary_view()
    proxy_name = proxy.name()
    top_level_event_handler_context.release()
    queries = top_level_event_handler_context.cleanup_released_object_proxies()
...
def test_cleanup_invalid_bucketfs_object_proxies_after_release(
    ...
    top_level_event_handler_context.release()
    top_level_event_handler_context.cleanup_released_object_proxies()
...
def test_cleanup_release_in_reverse_order_at_top_level(
        ...
        bucketfs_location: AbstractBucketFSLocation,
        prefix: str):
    proxies = [top_level_event_handler_context.get_temporary_table() for _ in range(10)]
    table_names = [proxy.name() for proxy in proxies]
    top_level_event_handler_context.release()
    query_objects = top_level_event_handler_context.cleanup_released_object_proxies()
...
def test_cleanup_release_in_reverse_order_at_child(
        ...
        bucketfs_location: AbstractBucketFSLocation,
        prefix: str):
    parent_proxies = [top_level_event_handler_context.get_temporary_table() for _ in range(10)]

    child = top_level_event_handler_context.get_child_event_handler_context()
    child_proxies = [child.get_temporary_table() for _ in range(10)]
    child_table_names = [proxy.name() for proxy in child_proxies]
    child.release()
    child_query_objects = top_level_event_handler_context.cleanup_released_object_proxies()
    ...
    parent_proxies.extend([top_level_event_handler_context.get_temporary_table() for _ in range(10)])
    parent_table_names = [proxy.name() for proxy in parent_proxies]
    top_level_event_handler_context.release()
    parent_query_objects = top_level_event_handler_context.cleanup_released_object_proxies()
