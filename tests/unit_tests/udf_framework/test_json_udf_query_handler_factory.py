...
class TestJSONQueryHandler(JSONQueryHandler):
    ...
    def __init__(
        self, parameter: JSONType, query_handler_context: ScopeQueryHandlerContext
    ):
...
def test(top_level_query_handler_context_mock):
    ...
    query_handler = TestJsonUDFQueryHandlerFactory().create(
        json_str, top_level_query_handler_context_mock
    )
    start_result = query_handler.start()
    handle_query_result = query_handler.handle_query_result(
        PythonQueryResult(
            data=[(1,)], columns=[Column(ColumnName("a"), ColumnType("INTEGER"))]
        )
    )
    assert (
        isinstance(query_handler, UDFQueryHandler)
        and start_result.result == json_str
        and handle_query_result.result == json_str
    )
