...
def test_from_existing_using_with_schema():
    ...
    table_name = (
        TableNameBuilder(table_name=source_table_name)
        .with_schema_name(SchemaName("schema"))
        .build()
    )
    assert (
        source_table_name.name == "table"
...
def test_from_existing_using_with_name():
    ...
    table_name = (
        TableNameBuilder(table_name=source_table_name).with_name("table1").build()
    )
    assert (
        source_table_name.name == "table"
...
def test_from_existing_and_new_schema_in_constructor():
    source_table_name = TableNameImpl("table")
    table_name = TableNameBuilder(
        schema=SchemaName("schema"), table_name=source_table_name
    ).build()
    assert (
        source_table_name.name == "table"
        and source_table_name.schema_name is None
...
def test_from_existing_and_new_name_in_constructor():
    source_table_name = TableNameImpl("table", SchemaName("schema"))
    table_name = TableNameBuilder(name="table1", table_name=source_table_name).build()
    assert (
        source_table_name.name == "table"
        and source_table_name.schema_name.name == "schema"
        and table_name.schema_name.name == "schema"
...
def test_using_create_name_using_only_name():
    ...
    assert table_name.name == "table" and isinstance(table_name, TableName)
