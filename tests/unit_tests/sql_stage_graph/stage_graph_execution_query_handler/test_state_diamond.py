...
def test_get_current_query_handler_diamond_return_finish_part1():
    ...
    def act(test_setup: TestSetup) -> QueryHandler[List[SQLStageInputOutput], SQLStageInputOutput]:
        result = test_setup.state_setup.execution_query_handler_state.get_current_query_handler()
        return result
...
def test_handle_result_diamond_return_finish_part6():
    ...
    assert_stage_train_query_handler_created(
        test_setup, stage_index=3,
        stage_inputs=[test_setup.stage_setups[1].results[0].result,
