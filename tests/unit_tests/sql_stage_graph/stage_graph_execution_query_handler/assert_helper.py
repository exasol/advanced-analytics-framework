...
def assert_reference_counting_bag_creation(test_setup: TestSetup):
    ...
    reference_counting_bag_mock_setup.factory.assert_called_once_with(parent_query_handler_context)
    assert reference_counting_bag_mock_setup.bag.mock_calls == []
...
def assert_stage_train_query_handler_created(
    ...
    stage_setup = test_setup.stage_setups[stage_index]
    mock_cast(test_setup.state_setup.result_bucketfs_location.joinpath).assert_called_once_with(str(stage_index))
    mock_cast(test_setup.state_setup.parent_query_handler_context.get_child_query_handler_context).assert_called_once()
    result_bucketfs_location = test_setup.stage_setups[stage_index].result_bucketfs_location
    ...
    mock_cast(stage_setup.stage.create_train_query_handler).assert_called_once_with(
        stage_input,
        stage_setup.child_query_handler_context
    )
    assert stage_setup.train_query_handler.mock_calls == []
    assert stage_setup.child_query_handler_context.mock_calls == []


def assert_release_on_query_handler_context_for_stage(test_setup: TestSetup, *, stage_index: int):
    stage_setup = test_setup.stage_setups[stage_index]
    assert stage_setup.train_query_handler.mock_calls == []
