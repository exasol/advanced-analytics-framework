...
MockObjectProxyReferenceCountingBagFactory = Union[ObjectProxyReferenceCountingBag, Mock]
...
def create_execution_query_handler_state_setup(
    ...
    child_scoped_query_handler_contexts = [stage_setup.child_query_handler_context
                                           for stage_setup in stage_setups]
    scoped_query_handler_context = create_mock_query_handler_context(child_scoped_query_handler_contexts)
    sql_stage_input_output: MockSQLStageInputOutput = create_autospec(SQLStageInputOutput)
    mock_result_bucketfs_location: MockBucketFSLocation = create_autospec(AbstractBucketFSLocation)
    stage_result_bucketfs_locations = [stage_setup.result_bucketfs_location
                                       for stage_setup in stage_setups]
    mock_cast(mock_result_bucketfs_location.joinpath).side_effect = stage_result_bucketfs_locations
...
def create_reference_counting_bag_mock_setup() -> ReferenceCountingBagSetup:
    reference_counting_bag: MockObjectProxyReferenceCountingBag = create_autospec(ObjectProxyReferenceCountingBag)
    ...
    return ReferenceCountingBagSetup(bag=reference_counting_bag,
                                     factory=reference_counting_bag_factory)


def create_mock_query_handler_context(child_scoped_query_handler_contexts: List[ScopeQueryHandlerContext]) \
        -> MockScopeQueryHandlerContext:
    scoped_query_handler_context: MockScopeQueryHandlerContext = \
        create_autospec(ScopeQueryHandlerContext)
    scoped_query_handler_context.get_child_query_handler_context.side_effect = child_scoped_query_handler_contexts
    return scoped_query_handler_context


def create_mocks_for_stage(result_prototypes: List[Union[Finish, Continue]], *, stage_index: int) -> StageSetup:
    child_scoped_query_handler_context: MockScopeQueryHandlerContext = \
        create_autospec(ScopeQueryHandlerContext)
    sql_stage: MockSQLStage = create_autospec(SQLStage)
    sql_stage.__hash__.return_value = stage_index
    result: List[MockQueryHandlerResult] = [create_autospec(result_prototype)
                                            for result_prototype in result_prototypes]
