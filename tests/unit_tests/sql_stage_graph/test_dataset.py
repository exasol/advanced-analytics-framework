...
def create_table_data_partition(
    name: str, columns: List[Column], dependencies: Dependencies = None
):
...
def test_dataset_partitions_with_same_table_like_name(identifier, sample, target):
    ...
    partition1 = create_table_data_partition(
        name="TRAIN", columns=[identifier, sample, target, extra_column]
    )
    ...
    with pytest.raises(
        ValueError,
        match="The names of table likes of the data partitions should be different.",
    ):
        Dataset(
            data_partitions={TestEnum.K1: partition1, TestEnum.K2: partition2},
...
def test_dataset_extra_column_valid(identifier, sample, target):
    ...
    partition2 = create_table_data_partition(
        name="TEST", columns=[identifier, sample, target, extra_column]
    )
    Dataset(
        data_partitions={TestEnum.K1: partition1, TestEnum.K2: partition2},
        identifier_columns=[identifier],
...
def test_dataset_partitions_different_columns_throws_exception(
    ...
    partition2 = create_table_data_partition(
        name="TEST", columns=[identifier, sample, target]
    )
    with pytest.raises(
        ValueError, match="Not all data partitions have the same columns."
    ):
        Dataset(
            data_partitions={TestEnum.K1: partition1, TestEnum.K2: partition2},
            identifier_columns=[identifier],
...
def test_dataset_not_contains_sample_throws_exception(identifier, sample, target):
    partition1 = create_table_data_partition(name="TRAIN", columns=[identifier, target])
    partition2 = create_table_data_partition(name="TEST", columns=[identifier, target])
...
def test_dataset_not_contains_target_throws_exception(identifier, sample, target):
    ...
    partition2 = create_table_data_partition(name="TEST", columns=[identifier, sample])
...
def test_dataset_not_contains_identifier_throws_exception(identifier, sample, target):
    partition1 = create_table_data_partition(name="TRAIN", columns=[target, sample])
    partition2 = create_table_data_partition(name="TEST", columns=[target, sample])
    with pytest.raises(
        ValueError, match="Not all identifier columns in data partitions."
    ):
        Dataset(
            data_partitions={TestEnum.K1: partition1, TestEnum.K2: partition2},
            identifier_columns=[identifier],
