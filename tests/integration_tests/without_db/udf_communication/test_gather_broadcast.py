...
class ConditionalEventSelection:
    def __init__(self, event: str):
        self._event = event

    def __call__(self, logger, method_name, event_dict):
        if event_dict["event"] != self._event:
            raise DropEvent
...
structlog.configure(
    ...
    processors=[
        ...
        ConditionalEventSelection("result"),
...
def run(parameter: CommunicatorTestProcessParameter,
    ...
    try:
        communicator = create_communicator(parameter)
        gather_input = f"{parameter.node_name}_{parameter.instance_name}".encode("utf-8")
        gather_result = communicator.gather(gather_input)
        broadcast_input = None
        if communicator.is_multi_node_leader():
            broadcast_input = str(gather_result).encode("utf-8")
        broadcast_result = communicator.broadcast(broadcast_input)
        LOGGER.info(
            "result",
...
def create_communicator(parameter: CommunicatorTestProcessParameter):
