    ...
    try:
        ...
        try:
            ...
            if com.peer.connection_info.name != "i0":
                time.sleep(10)
                peer_i0 = next(peer for peer in com.peers() if peer.connection_info.name == "i0")
                com.send(peer_i0, [socket_factory.create_frame(com.peer.connection_info.name.encode("utf8"))])
            else:
